...
desired_state(ClusterName)->
    AllDesiredState=desired_state(), %[{HostName,ClusterName,NumInstances,HostConstraints,PodConstraint,list_to_atom(ServiceId)}}]]
  %  io:format("AllDesiredState ~p~n",[{?MODULE,?LINE,AllDesiredState}]),
  %  io:format(" ~p~n",["**********************************************************"]),
    AllDeploymentDesiredState=[DeploymentDesiredState||
				  {CN,DeploymentDesiredState}<-AllDesiredState,
				  CN=:=ClusterName],
    lists:append(AllDeploymentDesiredState).
...
desired_state([DepName|T],Spec,Acc)->
    ...
    HostNames=lists:sort(deployment_data:item(hostnames,DepName,Spec)),
    HostConstraints=lists:sort(deployment_data:item(host_constraints,DepName,Spec)),

    PodConstraint=deployment_data:item(pod_constraints,DepName,Spec),
    ServiceIdVsnList=deployment_data:item(services,DepName,Spec),
    Result=desired_state(NumInstances,HostNames,ClusterName,HostConstraints,PodConstraint,ServiceIdVsnList,[]),
    NewAcc=[Result|Acc],
    desired_state(T,Spec,NewAcc).
    
      
desired_state(-1,_,ClusterName,_,_,_,Acc)->
    {ClusterName,Acc};
desired_state(NumInstances,HostNames,ClusterName,HostConstraints,PodConstraint,ServiceIdVsnList,Acc)->
    L=[{ClusterName,NumInstances,HostNames,HostConstraints,PodConstraint,list_to_atom(ServiceId)}||HostName<-HostNames,
												  {ServiceId,_}<-ServiceIdVsnList],
    NewAcc=lists:append(L,Acc),
    desired_state(NumInstances-1,HostNames,ClusterName,HostConstraints,PodConstraint,ServiceIdVsnList,NewAcc).
